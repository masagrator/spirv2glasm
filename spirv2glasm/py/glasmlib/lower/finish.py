...
def _ldc_line(l):
    """`^(LDC\.[A-Z]+\d+)(X[24])? (#\d+)(\.[xyzw]+)?, (.*)$` -> the five
...
def _sub_labels(l, fn):
    """`BB@(\d+)` replaced by `fn(digits)`."""
    return _lex.sub_numbered(l, "BB@", fn) if "BB@" in l else l


def _strip_read_masks(l):
    """`\b(R\d+)@[0-9a-f]\b` -> `\1`: the annotated read masks dropped."""
    if "@" not in l:
        return l
    out, pos = [], 0
...
def _drop_hidden_dest(l):
    """AN IMAGE STORE'S REGISTER (notes/111): the compiler's STOREIM node has
...
def _drop_release_tags(l):
    """... and its release-order tag (`@rhct`, image.py `_write_release`),
...
def _cc_set(line):
    """`^(\s*\S+?\.CC)(\s+[RH]C)((?:\.[xyzw]+)?\b)` matched: (group 1,
