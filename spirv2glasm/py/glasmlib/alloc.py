...
def _line_dst(l):
    r"""`^\s*\S+\s+([^,;]+?)\s*[,;]` matched: the destination, or None."""
    f = _first_field(l)
    if f is None:
        return None
    j, k = f
...
def _is_output_store(l):
    r"""A line that stores to a program OUTPUT, which is where a block ends:
