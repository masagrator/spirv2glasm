...
notes/33..37 establish:
    ...
    SPIR-V opcode
      -> py/data/body_workers.json   the worker's operator-code constant
      -> py/data/lowering_map.json   the IR operation FAMILY BASE   (tools/lowermap.py)
      -> py/data/ir_glasm_map.json   the GLASM opcode               (tools/iropmap.py)
...
# the measured tables the converter reads (they used to live in notes/,
# which ships as an archive now)
_NOTES = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data")
...
def _load_shape():
    """py/data/extinst_shape.json -- MEASURED, by tools/extshape.py.
    ...
    path = os.path.join(here, "data", "extinst_shape.json")
    try:
        with open(path) as f:
...
def extinst_single(number, type_code):
    ...
    if opcode in (RoundingOp.ROUNDING_6c, RoundingOp.ROUNDING_6d) and base:
        ...
        # mode (py/data/extinst_shape.json) and the type suffix is the ordinary
        # one.
        suf = suffix(opcode, type_code)
