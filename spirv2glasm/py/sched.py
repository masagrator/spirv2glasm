...
def _inner_pieces(operand):
    """`[\[(]([^\])]*)[\])]` findall: the text inside each bracket or
...
def _cc_test(t):
    """`^(NE|EQ|GT|GE|LT|LE)(1?)(\$\d+)?(\.[xyzw]+)?$` -> (one, tag,
...
def _release_tag(src):
    """`@r([htc]{3})\s*$` searched: the three letters, or None."""
...
def _predicated(dst):
    """`\((NE|..)(1?)(\$\d+)?(\.[xyzw]+)?\)\s*$` searched: the test
...
def _is_colour(name):
    """`result_color\d+$` matched at the start."""
...
def _is_barrier(line):
    """`^\s*(REP|..|CAL)\b|^BB\S*:|\.CC\s` MATCHED (at the start: the
