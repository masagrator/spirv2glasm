...
def _place(name):
    """The vreg of a placeholder `#n` (`#(\d+)` in full), or None."""
...
def _long_def(line):
    """`\s*(LDC\.U64|OR\.S)\s+D(\d+)\.x\s*,` matched: (the mnemonic,
...
def _cond_vreg(name):
    """`^C\$(\d+)$`: the condition vreg's number, or None."""
