...
def collapse_blanks(line):
    """`re.sub(r"[ \\t]+", " ", line)`: every run of spaces and tabs one
    space."""
...
def control_form(text):
    """A listing as GLSLC's fat control section stores it: the comment
    lines dropped, each run of blanks one space, trailing blanks and the
    final newlines gone.  The relation was established against the
    compiler's own stored sections (tools/checklisting.py, which uses this
    same definition)."""
    out = []
    for line in text.split("\n"):
        if line.startswith("#"):
            continue
        out.append(collapse_blanks(line).rstrip())
    return "\n".join(out).rstrip("\n")
